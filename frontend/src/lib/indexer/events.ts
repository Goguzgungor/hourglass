...
export interface ParsedEvent {
  ...
  // transactionIndex (transaction order within the ledger) and the RPC's
  // per-operation event ordinal, parsed from the `id` suffix — this stays
  // unique even when a single operation emits several events for the same
  // action (e.g. `create_batch`). The ordinal is unique per transaction only
  // because a Soroban transaction carries exactly one invoke-host-function
  // operation. Falls back to operationIndex when the id has no parsable
  // ordinal, which is only unique per operation.
  log_index: number;
  ts: number;
}

/**
 * Per-operation event ordinal from the RPC event id (`<toid>-<%010d>`).
 * Returns null when the id has no parsable suffix (older RPCs / synthetic
 * test events).
 */
...
export function parseEvent(e: rpc.Api.EventResponse): ParsedEvent | null {
  ...
  // transaction order first, then the RPC's per-operation event ordinal (from
  // the id suffix), which is unique per transaction because a Soroban
  // transaction has a single invoke-host-function operation. The 1_000_000
  // stride is safe: a ledger's per-transaction ordinals stay orders of
  // magnitude below it (a Soroban tx is capped well under a thousand events).
  // Falls back to the operation index for events without a parsable id, which
  // is only unique per operation.
