    ...
    <li className="grid grid-cols-[12px_1fr_auto] gap-x-3 items-baseline py-3 border-b border-stroke/40">
      ...
      <div className="min-w-0 font-mono text-xs">
        ...
        <p className="mt-1 text-[11px] text-cream-dim truncate">
          ...
          {row.self && <span> · to yourself</span>}
          {/* "by X · with X" says nothing twice: skip the counterparty when it is the actor already named. */}
          {row.counterparty && row.counterparty !== row.actor && (
            <span> · {row.kind === 'transferred' ? 'to' : 'with'} {truncAddress(row.counterparty)}</span>
          )}
        </p>
      </div>
