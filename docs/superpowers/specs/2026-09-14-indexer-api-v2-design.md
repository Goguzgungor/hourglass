...
- A retention error resets the state to `{ cursor: null, ledger: latest - STARTUP_BUFFER_LEDGERS }` **and schedules an immediate reconcile** (§6), because events were certainly missed. The public Soroban RPC rejects a stale `startLedger` and a stale `cursor` with the *same* JSON-RPC error — code `-32600`, message `startLedger must be within the ledger range: 4556625 - 4677584` (the SDK throws `response.data.error`, a plain object, so both the code and the message are read defensively). The older `oldest ledger` / `invalid cursor` phrasings are still matched; every other error rethrows.
- Reconcile scheduling is **independent of ingestion success**: the tick's `try/catch` wraps `fetchAndIngest` only, and the periodic/reset decision runs outside it. An RPC that fails every `getEvents` call must not also starve the self-healing path.
- A page that comes back without a `cursor` keeps the previous one (the page is simply re-fetched next tick — every write is idempotent). Falling back to `startLedger: latestLedger` would silently skip everything in between.
...
2. **Upsert missing / not-yet-depleted.** For each `id ∈ liveIds` where Mongo has no doc, or the doc is not `is_depleted`: `get_stream(id)` → upsert the materialized fields (same mapping as the `created` handler). A canceled stream is NOT terminal — withdrawals (and `withdraw_max_and_transfer`) stay legal until it is depleted — so only `is_depleted` excludes a doc from refresh. Docs discovered this way get `source: 'reconcile'`; `created_tx`/`created_ledger` stay unset and `created_at` defaults to `start_ts` when unknown (a schedule time, not a ledger time). `source` is written with `$setOnInsert` — it records who *first* materialized the doc, so a reconcile refresh of an event-ingested doc leaves `source: 'event'` alone. If the stream's `created` event is ingested later, its provenance is written with `$set` (a stream has exactly one `created` event, so it is authoritative) and `source` becomes `'event'`.
